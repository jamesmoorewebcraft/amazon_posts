...
- Do not repeat the same body-fit example, comfort threshold, missing feature or ventilation point in two adjacent subsections.
...
def _deterministic_semantic_claim_violations(
    ...
    compatibility_terms = [str(value).casefold() for value in controls.get(
        ...
        ["most airlines", "all airlines", "airline approved", "guaranteed carry-on", "guaranteed to fit"],
    )]
    for passage_index, passage in enumerate(passages):
        folded = passage.casefold()
